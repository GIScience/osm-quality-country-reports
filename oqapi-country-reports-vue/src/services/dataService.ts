...
export async function fetchAvailableCountries(): Promise<Country[]> {
  ...
  // Anonymous bucket listing is no longer allowed, so instead of listing the
  // bucket we fetch a manifest (generated by scripts/generate_downloads_index.py)
  // that lists the country folders actually present under oqapi_hdx/downloads/.
  const indexUrl = "https://hot.storage.heigit.org/heigit-hdx-public/oqapi_hdx/downloads/index.json";
  const resIndex = await fetch(indexUrl);
  const prefixes = ((await resIndex.json()) as string[])
    .map((code) => code.replace("oqapi_hdx/downloads/", "").replace(/\/$/, ""))
    .filter(Boolean);
