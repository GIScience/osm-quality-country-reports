...
def make_topic_gpkg_asset(topic: str):
    ...
    deps_ = {dep: dg.AssetIn(key=dep) for dep in deps}
    ...
    def generic_topic_gpkg_asset(context: dg.AssetExecutionContext, deps_) -> None:

        print(deps)
        dfs = [df for df in deps_ if df is not None]
        if not dfs:
            return None
