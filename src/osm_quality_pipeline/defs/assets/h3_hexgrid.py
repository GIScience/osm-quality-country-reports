...
from osm_quality_pipeline.defs import constants
...
@dg.asset(
    ins={"geoboundary_geojson": dg.AssetIn()}, partitions_def=country_partitions
)  # TODO: maybe change this to deps instead of ins
def h3_hexgrid(context, geoboundary_geojson: dg.Output[str]) -> dg.Output[str]:
    ...
    try:
        boundary_path = next(
            os.path.join(geoboundary_geojson, f)
            for f in os.listdir(geoboundary_geojson)
            if "ADM0" in f
        )
...
def create_h3_gdf(gdf, country):
    ...
    gdf = gpd.GeoDataFrame(
        gdf[["shapeName", "shapeISO", "geometry"]], geometry="geometry", crs="EPSG:4326"
    )
    grid_clipped = gpd.overlay(grid_gdf, gdf, how="intersection").reset_index(drop=True)

    grid_clipped["h3_id"] = f"{country}_hex{zoom_level}_" + (
